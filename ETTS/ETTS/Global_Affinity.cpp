...
//将分好词的句子拆分成词
void Global_Affinity::cut_word( vector<string> content, vector<vector<string>> &content_word ){
...
void Global_Affinity::calculate_tf( vector<vector<string>> content_word, vector< map<string,Word_frequency> > &tf ){
...
//计算idf
void Global_Affinity::calculate_idf( vector< map<string,Word_frequency> > tf, map<string,double> &idf ){
...
//计算余弦相似度（最后一个公式）
double Global_Affinity::calculate_PI( string word, int ID, vector< map<string, Word_frequency> > tf, map<string,double> idf, vector<int> &time ){
	double sum = 0;
	double key = 0;
	map<string, Word_frequency>::iterator iter_tf;
	map<string, double>::iterator iter_idf;
	...
	return ( time[ID] * key / sqrt( sum ) );
}
//计算Si与Sj的余弦相似度
void Global_Affinity::calculate_cos( vector< map<string, Word_frequency> > tf, map<string,double> idf, vector<vector<double>> &M, vector<int> &time ){
	vector<double> temp;
	for( int i = 0; i < tf.size(); i++ ){
		for( int j = 0; j < tf.size(); j++ ){
			...
			for( iter_tfi = tf[i].begin(); iter_tfi != tf[i].end(); iter_tfi++ ){
				...
				if( iter_tfj != tf[j].end() ){											//找到相同的词语
					sum = sum + calculate_PI( iter_tfi->first, i, tf, idf, time ) * calculate_PI( iter_tfi->first, j, tf, idf, time );
				}
			}
			temp.push_back( sum );
		}
...
//计算Si与Sj的余弦相似度
void Global_Affinity::calculate_cos( vector< map<string, Word_frequency> > tf_local, vector< map<string, Word_frequency> > tf_others,
										map<string,double> idf, vector<vector<double>> &M, vector<int> &time ){
	vector<double> temp;
	for( int i = 0; i < tf_local.size(); i++ ){
		for( int j = 0; j < tf_others.size(); j++ ){
			double sum = 0;
			if( i == j ){															//同一个句子，转移概率为0
				temp.push_back(0);
				continue;
			...
			map<string, Word_frequency>::iterator iter_tfi;
			map<string, Word_frequency>::iterator iter_tfj;
			for( iter_tfi = tf_local[i].begin(); iter_tfi != tf_local[i].end(); iter_tfi++ ){
				iter_tfj = tf_others[j].find( iter_tfi ->first );
				if( iter_tfj != tf_others[j].end() ){											//找到相同的词语
					sum = sum + calculate_PI( iter_tfi->first, i, tf_local, idf, time ) * calculate_PI( iter_tfi->first, j, tf_others, idf, time );
				}
			}
			temp.push_back( sum );
		}
...
//计算Si-->Sj的转移概率
void Global_Affinity::calculate_P( vector<vector<double>> &M ){
	for( int i = 0; i < M.size(); i++ ){
		double sum_row = 0;
		for( int j = 0 ; j < M[i].size(); j++ ){
			sum_row += M[i][j];
...
//输出转移矩阵数据
void Global_Affinity::print_M( vector<vector<double>> M ){
	ofstream out;
...
//计算矩阵M的转置
void Global_Affinity::transposition( vector<vector<double>> M, vector<vector<double>> &T ){
	int len = M.size();
	for( int i = 0; i < len; i++ ){
		vector<double> temp;
		for( int j = 0; j < len; j++ ){
...
//计算每个句子的可能性
void Global_Affinity::calculate_probability( vector<vector<double>> T, vector<double> &probability ){
	int len = T.size();
	vector<double> e;
	for( int i = 0; i < len; i++ ){
		e.push_back( 1 );
...
//两个矩阵相乘
vector<double> Global_Affinity::mul_matrix( vector<vector<double>> x, vector<double> y){
	int len = x.size();
...
//两个矩阵相乘
vector<double> Global_Affinity::mul_matrix( vector<double> x, vector<double> y){
	int len = x.size();
	vector<double> result;
	for( int i = 0; i < x.size(); i++ ){
		double sum = 0;
